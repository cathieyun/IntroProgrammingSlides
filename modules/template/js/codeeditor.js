/*
	...
	DO NOT LOOK IN HERE - FILE IS JUST A COMPILATION OF SUPPORTING UTILITIES
...
*/
	...
	window.onload = function(){
			slide = editorBuilder('code', 'canvas');
			slide.loadCode();				
		};
...
/* ------------------------- FILE: EDITORBUILDER.JS ---------------------------------- */

/* originally by Abel; extensively modified by DNJ */

var console_empty = true;

var output; // set below

console.log = function (message){
  print(message);
};

/* added by DNJ for REPL behaviour 
modified since last time
*/
print = function (v){
  if (!output) return;	// no output box displayed
	if (console_empty) {
        console_empty = false;
        output.innerHTML = "<pre>" + v + "</pre>";
    } else {
	    output.innerHTML += "<pre>" + v + "</pre>";
	}
};

var clearOutputBox = function (){
  if (!output) return;	// no output box displayed
  output.innerHTML = "<i>Output</i>";
  console_empty = true;
};
...
var timeStamp = function(){
  ...
  return "------- Time Stamp: " + hours + ":" + minutes + ":" + seconds + " -------";
}

var editorBuilder = function(codeElement, canvasElement){
	// assume DOM is now loaded
  ...
  if(document.getElementById(canvasElement)){
	...
	return {
		...
		loadCode: function (){
			if(typeof introEditor === "undefined")
			{
				introEditor = CodeMirror.fromTextArea(document.getElementById(codeElement), {
					mode: "javascript",
      ...
      if(document.getElementById(canvasElement)){      
        ...
        // log timestamp in output box
        // DNJ commented out timestamping
//        var output = document.getElementById("output");
//        output.innerHTML += "<br\> " +timeStamp();  

        // DNJ added this: always clear before showing output of a run
        clearOutputBox();
        
		//evaluate and load code
		// DNJ: added display of value
		var result = eval(introEditor.getValue());
        // only display value if not undefined; this makes cleaner output when using 
        // console.log.
        if (result !== undefined) print(result);
        if (console_empty) print("Undefined result");
			} catch (e) {
